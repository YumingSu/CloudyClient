describe("Home page", () => {
  ...
  it("Shows a header", () => {
    cy.get(".vue-title").should('contain','Face a trouble with Fakes ??');
  });

  describe("Navigation bar", () => {
      it("Shows the required links", () => {
        cy.get(".navbar").find('[data-test=reportbtn]' );
        ...
        cy.get('.navbar-nav:nth-child(1)').within(() => {
          ...
          cy.get('.nav-item:nth-child(3)').should('contain', 'Report')
      ...
      it("Redirects when links are clicked", () => {
        cy.get('[data-test=reportbtn]').click();
        cy.url().should('include','/report' )
        cy.get(".navbar").contains('Manage' ).click();
        cy.url().should('include','/imitations' )
        // etc
      });
    })
});
